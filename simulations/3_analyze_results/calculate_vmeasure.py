...
def find_clustered_files(data_dir, method_filter=None, batch_filter=None):
    """
    Find all trajectories.clust.tsv files.
    ...
    # Pattern: data/{N}_classes/{functions}/noise_{X}/seed_{Y}/clustering/{method}_k{k}/trajectories.clust.tsv
    for clust_file in data_path.glob('*_classes/*/*/*/clustering/*/trajectories.clust.tsv'):
        parts = clust_file.parts

        # Extract metadata from path
        classes_idx = [i for i, p in enumerate(parts) if '_classes' in p][0]
        classes_dir = parts[classes_idx]
        num_classes = int(classes_dir.split('_')[0])

        # Skip if batch filter specified
        if batch_filter and classes_dir != batch_filter:
            continue

        func_combo = parts[classes_idx + 1]
        noise_dir = parts[classes_idx + 2]
        seed_dir = parts[classes_idx + 3]
        method_dir = parts[classes_idx + 5]  # e.g., "gmm_k3"
...
def main():
    ...
    parser.add_argument(
        '--output',
        type=str,
        default='../results/vmeasure_scores.tsv',
    ...
    for file_info in files:
        desc = f"{file_info['classes_dir']}/{file_info['function_combo']}/noise_{file_info['noise_level']:.2f}/seed_{file_info['seed']:03d}"
