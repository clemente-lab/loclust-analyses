#!/bin/bash
#BSUB -J r_mfuzz[1-1740]
#BSUB -o logs/r_mfuzz.out
#BSUB -e logs/r_mfuzz.err
#BSUB -n 1
...
#
# Usage:
#   # Replace mfuzz in the script with actual method name, then:
#   bsub < run_r_kml.sh
...
# mfuzz MUST BE SET - this is a template
mfuzz="mfuzz"
...
OUTPUT_DIR="${TRAJ_DIR/data_potentially_simple/data_minimal_features}/clustering/${mfuzz}_k${NUM_CLASSES}"
mkdir -p "$OUTPUT_DIR"
...
Rscript "$SINGLE_R_SCRIPT" "$mfuzz" "$TRAJ_FILE" "$NUM_CLASSES" "$OUTPUT_DIR" "$OUTPUT_BASENAME"
