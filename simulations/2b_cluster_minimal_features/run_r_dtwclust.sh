#!/bin/bash
#BSUB -J r_dtwclust[1-1740]
#BSUB -o logs/r_dtwclust.out
#BSUB -e logs/r_dtwclust.err
#BSUB -n 1
...
#
# Usage:
#   # Replace dtwclust in the script with actual method name, then:
#   bsub < run_r_kml.sh
...
# dtwclust MUST BE SET - this is a template
dtwclust="dtwclust"
...
OUTPUT_DIR="${TRAJ_DIR/data_potentially_simple/data_minimal_features}/clustering/${dtwclust}_k${NUM_CLASSES}"
mkdir -p "$OUTPUT_DIR"
...
Rscript "$SINGLE_R_SCRIPT" "$dtwclust" "$TRAJ_FILE" "$NUM_CLASSES" "$OUTPUT_DIR" "$OUTPUT_BASENAME"
