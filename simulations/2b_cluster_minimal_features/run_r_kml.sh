#!/bin/bash
#BSUB -J r_kml[1-1740]
#BSUB -o logs/r_kml.out
#BSUB -e logs/r_kml.err
#BSUB -n 1
...
#
# Usage:
#   # Replace kml in the script with actual method name, then:
#   bsub < run_r_kml.sh
...
# kml MUST BE SET - this is a template
kml="kml"
...
OUTPUT_DIR="${TRAJ_DIR/data_potentially_simple/data_minimal_features}/clustering/${kml}_k${NUM_CLASSES}"
mkdir -p "$OUTPUT_DIR"
...
Rscript "$SINGLE_R_SCRIPT" "$kml" "$TRAJ_FILE" "$NUM_CLASSES" "$OUTPUT_DIR" "$OUTPUT_BASENAME"
